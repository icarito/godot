...
bool CIDRPolicy::is_allowed(const IP_Address &p_ip) {
...
String CIDRPolicy::describe_block(const IP_Address &p_ip) {
	if (force_fail_active()) {
	...
	if (is_v4_mapped) {
		...
		for (const V4CIDR &cidr : v4_blocklist) {
			if (match_v4(v4_host, cidr)) {
				return tg_vformat("%s/%d (private/loopback/link-local)", String(p_ip), cidr.prefix);
			}
		}
	}
	for (const V6CIDR &cidr : v6_blocklist) {
