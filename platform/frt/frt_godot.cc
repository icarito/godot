...
#ifdef GLAD_ENABLED
			Error err = RasterizerGLES2::is_viable();
			if (err != OK)
				fatal("GLES2 renderer is not viable on this context.");
			print_verbose(vformat("GL_VERSION: %s", (const char *)glGetString(GL_VERSION)));
#else
...
#ifdef GLAD_ENABLED
			Error err = RasterizerGLES3::is_viable();
			if (err != OK)
				fatal("GLES3 renderer is not viable on this context.");
			print_verbose(vformat("GL_VERSION: %s", (const char *)glGetString(GL_VERSION)));
#else
...
public: // OS
	...
	Error get_entropy(uint8_t *buffer, int bytes) override {
		int r = open("/dev/urandom", O_RDONLY);
		ERR_FAIL_COND_V(r < 0, FAILED);
		int left = bytes;
		do {
			ssize_t ret = read(r, buffer, bytes);
	...
	void set_mouse_mode(OS::MouseMode mode) override {
		// Under --no-window the launcher's visible window owns the pointer and
		// forwards input. The renderer still records the mode so the change is
		// relayed to the launcher, but it must not grab its own invisible
		// window: that starved the browser chrome of clicks.
		os_.set_mouse_mode(map_mouse_mode(mode), !is_no_window_mode_enabled());
	}
	...
	void pump_events_keepalive() override {
		// SDL only: answers Wayland's xdg_wm_base.ping while leaving queued input
		// and window events for the regular dispatch (see OS_FRT::pump_events).
		os_.pump_events();
	}
...
public: // EventHandler
	...
	void handle_focus_event(bool p_gained) override {
		if (p_gained) {
			if (main_loop_)
				main_loop_->notification(MainLoop::NOTIFICATION_WM_FOCUS_IN);
		} else {
			input_->release_pressed_events();
			if (main_loop_)
				main_loop_->notification(MainLoop::NOTIFICATION_WM_FOCUS_OUT);
		}
	}
	void handle_mouse_enter_leave_event(bool p_entered) override {
		if (main_loop_)
			main_loop_->notification(p_entered ? MainLoop::NOTIFICATION_WM_MOUSE_ENTER : MainLoop::NOTIFICATION_WM_MOUSE_EXIT);
	}
};
